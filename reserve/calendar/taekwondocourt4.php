...
function selectTime(button) {
    ...
    function updateButtonStyling(timeData) {
        ...
        timeButtons.forEach(button => {
            const timeInfo = timeData.find(item => item.time === button.textContent && item.court_number === 'court 4');
            if (timeInfo) {
                button.style.backgroundColor = 'red';
            }
        });
    ...
    function updateCourtNumber(selectElement) {
            ...
            if (selectedCourt === 'court 5') {
                window.location.href = 'taekwondocourt5.php'; // Redirect to arniscourt2.php
