<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql = "INSERT INTO reservation_payments (username, sports, date, time, court_number, duration, promo_code, reference_no, gcash_qrcode, total) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssssssd", $logged_on_user, $sport, $date, $time, $court_number, $duration, $promo_code, $reference_no, $gcash_qrcode, $total); // Updated
        $stmt->execute();
        $stmt->close();
